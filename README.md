...
## Features
### Current features:
1. Support for monitroing up to 5 accounts concurrently <sub>(Twitter API limit, workaround possible if userbase needed it)</sub>.
...
3. Stream real-time tweets from monitored accounts and analyze them.
   ...
   - Support for sending tweeted links to OSINT for analysis <sub>(only VT so far)</sub>.
6. Intelligently set Twitter API query frequency to avoid hitting API rate limits.
7. Maximum detection delay: 12 seconds <sub>(it's 10 seconds but Twitter rate limits are inconsistent sometimes)</sub>.
...
3. Give options to ignore alerting on some monitors <sub>(e.g. don't alert for changes in the profile picture)</sub>.
...
9. Support for more TI & OSINT platforms <sub>(urlscan integration with nice features is upcoming)</sub>.
## Requirements
1. Python packages:
   - colorama==0.4.6
   - requests==2.28.1
   ...
   - urlextract==1.7.1
2. API keys:
   - Twitter bearer token <sub>(any access is fine)</sub>
   ...
   - VirusTotal API <sub>(free is fine)</sub>
